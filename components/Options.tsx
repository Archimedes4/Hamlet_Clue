  ...
  return (
    <View style={{width: width, height: height, backgroundColor: '#fcba03'}}>
      <ScrollView>
        ...
        <DefaultButton style={{flexDirection: 'row', width: width * 0.8, padding: 10, marginLeft: 'auto', marginRight: 'auto', marginTop: 5, height: 33}} textStyle={{margin: 0, fontSize:9, marginLeft: 2.5, marginRight: 'auto', height: 'auto'}} onPress={() => {store.dispatch(screensSlice.actions.hideAllScreens()); store.dispatch(screensSlice.actions.setDetectiveSheetScreen(true))}} text='Detective Notes' Icon={({hover}:{hover: boolean}) => <MagnifyingGlass width={9} height={9} color={hover ? 'white':'black'} style={{marginLeft:'auto'}}/>}/>
